...
class PyrLK3:
    ...
    def _calc_flow_pt(prev_pt, init_p, prev_vol, next_vol, win_size, min_eps, max_iter):
        ...
        template = PyrLK3.cut_volume(prev_vol, prev_pt, win_size)

        for i in range(max_iter):
            # warp point
            prev_pt_w = PyrLK3.warp(prev_pt, p)

            # cut out template at warped point
            warped = PyrLK3.cut_volume(next_vol, prev_pt_w, win_size)

            # compute image derivatives
            vx, vy, vz, vt = PyrLK3.compute_derivatives(template, warped)
